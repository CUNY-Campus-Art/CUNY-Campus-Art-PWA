...
const mapDispatch = (dispatch: any) => {
  return {
    ...
    editArtwork: (artwork: any, pic: any, artworkId: any) =>
      dispatch(editUploadedArtworkThunk(artwork, pic, artworkId)),
...
const UploadArtwork1 = (props: any) => {
  ...
  let artwork =
    Object.keys(props.artwork).length > 0
      ? props.artwork
      : {
          title: "",
          artist: "",
  ...
  let formValues: any = {
    ...
    title: props.artwork?.title ? props.artwork.title : "",
    artist: props.artwork?.artist ? props.artwork.artist : "",
    description: props.artwork?.description ? props.artwork.description : "",
    year: props.artwork?.year ? props.artwork.year : "",
  };
  ...
  const [selectedCampus, setSelectedCampus] = useState<string>(
    artwork != null ? artwork.campus : ""
  );

  const [campusSelected, setCampusSelected] = useState(
    artwork != null ? true : false
  );

  const [imgData, setImgData] = useState(
    artwork != null ? artwork.primary_image : null
  );
      ...
      {!success ? (
        <IonContent>
          <form onSubmit={handleSubmit(handleSubmit1)}>
            ...
            <IonItem>
              {edit && artwork && artwork.primary_image?.url ? (
                <img width="50px" src={artwork.primary_image.url}></img>
              ) : (
                <></>
              )}
              ...
              >
                {props.campuses
                  ? props.campuses.map((campus: any, index: any) => (
                      <IonSelectOption key={index} value={campus.id}>
                        {campus.campus_name}
                      </IonSelectOption>
                    ))
                  : ""}
            ...
            {campusSelected ? (
              ...
              <IonText color="danger" className="ion-padding-start">
                <small>
                  <span role="alert">Please select your campus</span>
                </small>
              </IonText>
            )}
            ...
            {!loading ? (
              ...
              <div className="spin">
                <IonSpinner color="success">Loading</IonSpinner>
              </div>
            )}
          </form>
        </IonContent>
          ...
          <IonGrid>
            ...
            <IonRow>
              <IonCol>
                <ArtworkCard
                  {...props}
                  isFromSuccessView={true}
                  deleteArtwork={props.deleteArtwork}
                  artwork={props.uploadedArtwork}
                ></ArtworkCard>
              </IonCol>
            </IonRow>
            <IonRow>
              <IonCol style={{ textAlign: "center" }}>
                <IonButton onClick={() => uploadNewArtwork()}>
